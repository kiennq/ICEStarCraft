...
{
	//_T_
	if (type == BWAPI::UnitTypes::Terran_Factory || type == BWAPI::UnitTypes::Terran_Starport)
	{
		if (buildDist == 0)
		{
			buildDist = 1;
	...
	if (type.isRefinery() || type == BWAPI::UnitTypes::Terran_Bunker || type == BWAPI::UnitTypes::Terran_Missile_Turret)
	{
		buildDist = 0;
	}//_T_
	...
	//if (!this->canBuildHere(position, type)) //_T_
	if (!this->canBuildHere(position, type) && !type.isAddon())
	{
		return false;
	}
	...
	if (!type.isRefinery())
	{
		for(int x = startx; x < endx; x++)
			for(int y = starty; y < endy; y++)
				...
				//_T_
				if (!buildable(x, y) || (reserveMap[x][y] && !type.isAddon()))
				{
					return false;
				}
	}
