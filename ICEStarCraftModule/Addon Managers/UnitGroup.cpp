...
UnitGroup& UnitGroup::operator*=(const UnitGroup& other)
{
  ...
  for(set<Unit*>::const_iterator i=this->begin();i!=this->end();i=i2)
  {
    i2=i;
    i2++;
    if (!other.contains(*i))
...
void UnitGroup::initMining(BWAPI::TilePosition s, BWAPI::TilePosition e)
{
	if (empty() || s == TilePositions::None || e == TilePositions::None)
	{
		return;
	}

	miningPath = getShortestPath(e,s);
  std::reverse(miningPath.begin(), miningPath.end());
	if (!miningPath.empty())
	{
		current = 0;
		dir = 1;
...
void UnitGroup::doMining( double skip /*= 3*/ )
{
	int count = 0;
	if (empty() || miningPath.empty()) return;

	//for (vector<TilePosition>::iterator i = miningPath.begin(); i != --miningPath.end(); i++) {
	//	vector<TilePosition>::iterator j = i;
	//	j++;
	//	//Broodwar->drawLineMap(i->x()*32+15, i->y()*32+15, j->x()*32+15, j->y()*32+15, Colors::Red);
	//}

	if (current+1>= (int)miningPath.size() || current < 0) {
		dir = -dir;
		current += dir*skip;
	}
	//Broodwar->printf("Current: %.1lf",current);

	/*Position center = getCenter();
	Position sampleUnit = (*begin())->getPosition();
	int dis = center.getApproxDistance(sampleUnit);*/
	for (UnitGroup::iterator i = this->begin(); i != this->end(); i++)
	{
		if (!(*i)->isCompleted()) continue;
		//UnitGroup en = SelectAllEnemy()(isVisible, canAttack).not(isBuilding).inRadius(UnitTypes::Terran_Vulture.sightRange(), (*i)->getPosition());
		//en += SelectAllEnemy()(isBuilding, canAttack).inRadius(UnitTypes::Terran_Vulture.sightRange(), (*i)->getPosition());
		//_T_
		UnitGroup en = SelectAllEnemy()(canAttack).inRadius(UnitTypes::Terran_Vulture.sightRange(), (*i)->getPosition());

		//Broodwar->drawCircleMap((*u)->getPosition().x(), (*u)->getPosition().y(), UnitTypes::Terran_Vulture.sightRange(), Colors::Orange);
		//bool inRange = false;
		// Check if there is enemy in range
		//for (UnitGroup::iterator e = en.begin(); e!=en.end(); e++) {
		//if (!inRadius((*e)->getType().groundWeapon().maxRange(), (*e)->getPosition()).empty()) {
		if (!en.empty() && Broodwar->getFrameCount()%24 == 0)
		{
			//inRange = true;
			dir = -1;
			current -= skip*5;
	...
	else if (current+1>= (int)miningPath.size())
	{
		current = miningPath.size()-1;
		return;
	}

	int next = current + 2 >= miningPath.size() ? (int)(current-1) : (int)(current+1);
	if (next < 0) next = (int)current;
	int dirX = miningPath[(int)current].y() - miningPath[next].y();
	int dirY = miningPath[next].x() - miningPath[(int)current].x();
	if (dirX==0 && dirY==0) dirX=dirY=1;
	
	for (UnitGroup::iterator _u=this->begin(); _u!=this->end(); _u++) {
		Unit* u = *_u;
		//&& (Broodwar->getFrameCount()- u->getLastCommandFrame() > 24*1)
		if ((u->isCompleted() && u->getType()==UnitTypes::Terran_Vulture  && (u->getOrder()!=Orders::PlaceMine ||u->isIdle() || Broodwar->getFrameCount()- u->getLastCommandFrame()>24*1))){
			if (Broodwar->self()->hasResearched(TechTypes::Spider_Mines)&&u->getSpiderMineCount()>0){
				
				if (u->getOrder() == Orders::PlaceMine ||  !u->isIdle()){
					int d = u->getPosition().getApproxDistance(u->getTargetPosition());
					if (d > 32*2 || isTileWalkable(TilePosition(u->getTargetPosition()))) continue;
				}
				...
				TilePosition randomPosition(miningPath[(int)current]);
				int countdown = 10;
				do {
					int rangeX = dirX*(-(int)size()/2 + rand()%(size()));// generate a random number between 50-65
					int rangeY = dirY*(-(int)size()/2 + rand()%(size()));
					randomPosition += TilePosition(rangeX, rangeY);
					countdown--;
				} while (!isTileWalkable(randomPosition) && countdown > 0);
				//Broodwar->drawCircleMap(randomPosition.x()*32 + 16, randomPosition.y()*32 + 16, 3, Colors::Blue,true);
				if (u->getPosition().getApproxDistance(Position(randomPosition)) <= 32*3) {
					u->useTech(TechTypes::Spider_Mines, u->getPosition());
					current += dir*skip;
				}
				else u->useTech(TechTypes::Spider_Mines, Position(randomPosition));
...
bool UnitGroup::isMining()
{
	return !miningPath.empty();
}

bool UnitGroup::attackMove( BWAPI::Position position, bool withLead /*= true*/ )
{
	bool retval=true;
	if (withLead && target && *target!=position) {
		target = PositionPtr(new Position(position.x(), position.y()));
		lead = NULL;
		int minDist = 10000;
		for(set<Unit*>::const_iterator i=this->begin();i!=this->end();i++)
		{
			int dist = (*i)->getPosition().getApproxDistance(*target);
			if (dist < minDist) {
				minDist = dist;
				lead = (*i);
			}
...
bool UnitGroup::move( BWAPI::Position position, bool withLead /*= false*/ )
{
	bool retval=true;
	if (withLead && target && *target!=position) {
		target = PositionPtr(new Position(position.x(), position.y()));
...
Position UnitGroup::getTargetPosition()
{
	Position target = Position(0,0);
	int count = 0;
	for (set<Unit*>::const_iterator i = this->begin(); i != this->end(); i++)
	{
		if ((*i)->getOrderTarget() && (*i)->getOrderTarget()->exists() && (*i)->getOrderTarget()->getPosition() != Positions::None)
		{
			target += (*i)->getOrderTarget()->getPosition();
			count++;
		}
		else if ((*i)->getOrderTargetPosition() != Positions::None)
		{
			target += (*i)->getOrderTargetPosition();
			count++;
		}
	}
	...
	if (count > 0)
	{
		return Position(target.x()/count,target.y()/count);
	}
	return Positions::None;
}