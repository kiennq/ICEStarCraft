					...
					else //if we have a worker
					{
						...
						if (!u->isConstructing()) //if the worker isn't constructing
						{
							...
							if (distance > 64 && u->getLastCommandFrame() + 4 < BWAPI::Broodwar->getFrameCount() && u->getOrder() != BWAPI::Orders::Move ) //if its too far away, tell it to go to the build site
								...
								{
									//we want to build command center is built on right position, not anywhere
									if (b->type == UnitTypes::Terran_Command_Center)
									{
										//get unit set on tile
										//std::set<Unit*> unitsOnSpot = Broodwar->getUnitsInRadius(b->position,32*3);
										std::set<Unit*> unitsOnSpot = Broodwar->getUnitsInRectangle(Position(b->tilePosition),Position(b->tilePosition + TilePosition(b->type.tileWidth(),b->type.tileHeight())));
										for each (Unit* un  in unitsOnSpot)
										{
											//if this unit is ours
											if (un->getPlayer() == Broodwar->self())
											{
												//if this unit is not a building, then move away
												if (!un->getType().isBuilding())
												{
													//if the unit is vulture mine, then destroy it
													if(un->getType() == UnitTypes::Terran_Vulture_Spider_Mine)
													{														
														UnitGroup tempAttackGroup = SelectAll()(isCompleted)(canAttack).not(isWorker,isBuilding,Vulture_Spider_Mine);
														//if (!u->isConstructing())
														u->attack(un);
														if (!tempAttackGroup.empty())
														{
															Unit* mineCleanner = (*tempAttackGroup.begin());
															mineCleanner->attack(un);
														}
													}
													//if the unit is the builder or mining worker
													else if (un == b->builderUnit || (un->getType().isWorker() && (un->isGatheringGas() || un->isGatheringMinerals())))
														continue;
													//if is sieged tank
													else if (un->getType() == UnitTypes::Terran_Siege_Tank_Siege_Mode)
														un->unsiege();													
													else
													{
														un->move(BWTA::getStartLocation(Broodwar->self())->getPosition());									
													}
												}
												...
												{
													//unfinished unit
													if (!un->isCompleted() && un->getType()!=UnitTypes::Terran_Command_Center && !un->getType().isAddon()
														&& un->getType()!=UnitTypes::Terran_Refinery)
														un->cancelConstruction();
													//for a completed building
													else if (un->isCompleted()){
														//if this unit can lift, then return
														if (un->getType()!=UnitTypes::Terran_Command_Center && un->lift())															
															continue;
	
														else if (un->getType()==UnitTypes::Terran_Command_Center || un->getType().isAddon()
															|| un->getType()==UnitTypes::Terran_Refinery)
															continue;
														//if the building can not lift,then destroy it
														else{
															UnitGroup tempAttackGroup = SelectAll()(isCompleted)(canAttack).not(isWorker,isBuilding,Vulture_Spider_Mine);
                                ...
                                WorkerManager::create()->addToNotRepairList(un);
											...
											{
												//if it is a neutral building or unit
												if (un->getPlayer()->isNeutral())
												{
													if (un->isInvincible())
													{
														this->placer->freeTiles(b->tilePosition, b->type.tileWidth(), b->type.tileHeight());
													...
													{
														UnitGroup tempAttackGroup = SelectAll()(isCompleted)(canAttack).not(isWorker,isBuilding);
														if (Broodwar->getFrameCount()%24==0)
														{
															if(!u->isConstructing())
																u->attack(un);
												...
												{
													// invisible units: dark templar...
													if (!un->isDetected())
													{
														UnitGroup scanners = SelectAll()(isCompleted)(Comsat_Station)(Energy,">=",50);
														if (!scanners.empty())
														{
															Unit* scanner = *(scanners.begin());
															scanner->useTech(TechTypes::Scanner_Sweep,un->getPosition());
														}
													}
													else
													{
