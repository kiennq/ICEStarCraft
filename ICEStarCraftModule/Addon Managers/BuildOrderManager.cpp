...
BuildOrderManager::BuildOrderManager(BuildManager* buildManager, TechManager* techManager, UpgradeManager* upgradeManager, WorkerManager* workerManager, SupplyManager* supplyManager)
{
	...
	this->existBaseOrder.clear();
	this->lastExpandLocation=BWTA::getStartLocation(Broodwar->self());
	this->planExpandLocation=NULL;
}

BuildOrderManager::BuildOrderManager(BuildManager* buildManager,SupplyManager* supplyManager,WorkerManager* workerManager)
{
	buildOrderManager  = this;
	this->buildManager       = buildManager;
	this->supplyManager      = supplyManager;
  ...
  // Besure to destroy mine in the commandCenter site before hand
	if (time%(24*10) == 11)
	{
		for (map<TilePosition,int>::iterator i = this->plannedCommandCenter.begin(); i != this->plannedCommandCenter.end();)
		{
			...
			bool hasBuiltCC = false;
			for each (Unit* u in Broodwar->getUnitsInRadius(Position(tp),32*4))
			{
				if (u->getPlayer() == Broodwar->self() && u->getType() == UnitTypes::Terran_Command_Center)
				{
					hasBuiltCC = true;
					break;
				}
			...
			if (hasBuiltCC)
			{
				i = this->plannedCommandCenter.erase(i);
			}
			else
			{
				if (time - i->second > 24*30 && Broodwar->self()->minerals() > UnitTypes::Terran_Command_Center.mineralPrice() + 100)
				{
					if (Broodwar->self()->completedUnitCount(UnitTypes::Terran_Comsat_Station) < 1)
					{
						this->build(1,UnitTypes::Terran_Comsat_Station,100);
						break;
					}
					else if (SelectAll(UnitTypes::Terran_Comsat_Station)(Energy,">=",50).empty())
					{
						break;
					}
					else
					{
						if (SelectAll()(isCompleted)(canAttack).inRadius(32*6,Position(tp)).empty())
						{
							// no combat units here, don't scan
						}
						else
						{
							// scan to see if there are mines here
							Unit* scanner = *(SelectAll(UnitTypes::Terran_Comsat_Station)(Energy,">=",50).begin());
							scanner->useTech(TechTypes::Scanner_Sweep,Position(tp));
							this->plannedCommandCenter[tp] = time;
						}
					}
				}
				else if (Broodwar->self()->minerals() <= UnitTypes::Terran_Command_Center.mineralPrice() + 100)
				{
					this->plannedCommandCenter[tp] = time;
				}
				i++;
	...
	for(list<Type>::iterator i=this->savedPlan.begin();i!=this->savedPlan.end();i=i2)
	{
		...
		if (unit!=UnitTypes::None)
		{
			...
			if (ctime<=time && hasResources(unit,btime))
			{
				...
				//_T_
				if (unit == UnitTypes::Terran_Command_Center)
				{
...
void BuildOrderManager::build(int count, BWAPI::UnitType t, int priority, BWAPI::TilePosition seedPosition, bool buildWithSpace)
{
	...
	{
		if (!this->buildManager->getBuildingPlacer()->canBuildHereWithSpace(seedPosition, t, space))
			seedPosition = this->buildManager->getBuildingPlacer()->getBuildLocationNear(seedPosition, t, space);
	}
...
//_T_
bool BuildOrderManager::plannedTech(TechType t)
{
	return this->techManager->planned(t);
}
...
void BuildOrderManager::setBaseManagerClass(BaseManager* bmc)
{
	this->bmc = bmc;
}

void BuildOrderManager::autoExpand(int priority,int maxlimit)
{
	...
	{
		buildAdditional(1,BWAPI::UnitTypes::Terran_Command_Center,priority,this->planExpandLocation->getTilePosition());
		this->existBaseOrder.insert(this->planExpandLocation->getTilePosition());
		this->lastExpandLocation = this->planExpandLocation;
	}
	else
	{
		checkDistance = false;
...
void BuildOrderManager::onUnitDestroy(BWAPI::Unit* u)
{
	if (u->getPlayer()==Broodwar->self()&&u->getType()==UnitTypes::Terran_Command_Center){
		if (this->existBaseOrder.find(u->getTilePosition())!=this->existBaseOrder.end()){
			this->existBaseOrder.erase(u->getTilePosition());
		}
	}
}